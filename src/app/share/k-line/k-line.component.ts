...
@Component({
  ...
  styleUrls: ['./k-line.component.css']
})
export class KLineComponent implements OnInit {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (this.inputData && this.inputData.status) {
      this.generateChart();
    }
  }
  ...
  generateChart(){
    ...
    this.options = {
      legend: {
        ...
        align: 'left',
        selected: { '開盤價': false, '收盤價': false, '最高價': false, '最低價': false, 'MA20': false, 'MA30': false }
      },
      tooltip: {
        confine: true,
        trigger: 'axis',
  ...
  splitData() {
    ...
    this.inputData.result.forEach((element: any) => {
      this.categoryData.push(element.yearMonthDate);
      let valueArray = [];
